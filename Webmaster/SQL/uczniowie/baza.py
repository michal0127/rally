...
def main(args):
    ...
    for tab in table:
        ile = ile_kolumn(tab)  # ile mamy pól
        dane = dane_z_pliku(tab + '.csv')
        ile_d = len(dane[0])
        if ile > dane_d: # primary key autoincrement
        ...
        cur.execumentary('INSERT INTO ' + tab + ' VALUES(' + pholders + ')', dane) 
